      ...
      <section className="space-y-4 rounded-xl border border-slate-700/50 bg-slate-800/60 p-4 sm:p-5">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-300">
            {t({ ko: "API 프로바이더", en: "API Providers", ja: "API プロバイダー", zh: "API 提供方" , th: "API Providers" })}
          </h3>
        ...
        <p className="text-xs text-slate-500">
          {t({
            ...
            th: "Register local, frontier, and compatible third-party APIs, then assign those models directly to agents.",
          })}
        </p>
        ...
        {apiAddMode && (
          <div className="space-y-3 rounded-lg border border-blue-500/30 bg-slate-900/50 p-4">
            <h4 className="text-xs font-semibold uppercase text-blue-400">
              {apiEditingId
                ? t({ ko: "프로바이더 수정", en: "Edit Provider", ja: "プロバイダー編集", zh: "编辑提供方" , th: "แก้ไขผู้ให้บริการ" })
                : t({
                    ko: "새 프로바이더 추가",
                    en: "Add New Provider",
                    ja: "新しいプロバイダーを追加",
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400">
                {t({ ko: "공식 프리셋", en: "Official Presets", ja: "公式プリセット", zh: "官方预设" , th: "Official Presets" })}
              </label>
              <p className="mb-2 text-[11px] text-slate-500">
                {t({
                  ...
                  th: "Official presets for OpenCode Go and Bailian Coding Plan.",
                })}
              </p>
              ...
              {apiPresetsLoading ? (
                ...
                <div className="rounded-lg border border-slate-700/40 bg-slate-900/40 px-3 py-2 text-[11px] text-slate-500">
                  {t({
                    ...
                    zh: "无法加载官方预设。",
                    th: "Official presets could not be loaded.",
                  })}
                </div>
              ) : (
            ...
            {selectedOfficialPreset && (
              <div className="rounded-lg border border-blue-500/30 bg-blue-950/20 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  >
                    {t({ ko: "문서 열기", en: "Open docs", ja: "ドキュメントを開く", zh: "打开文档" , th: "Open docs" })}
                  </a>
                </div>
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400">
                {t({ ko: "일반 타입", en: "Generic Type", ja: "汎用タイプ", zh: "通用类型" , th: "Generic Type" })}
              </label>
              <p className="mb-2 text-[11px] text-slate-500">
                {isOfficialPresetSelected
                  ? t({
                      ...
                      th: "Choose a generic type to leave preset mode and unlock manual editing.",
                    })
                  : t({
                      ...
                      th: "Use a generic type when you want to manage protocol and Base URL yourself.",
                    })}
              </p>
              ...
              {isOfficialPresetSelected && (
                <p className="mt-1 text-[11px] text-slate-500">
                  {t({
                    ...
                    th: "The selected preset manages the protocol and Base URL.",
                  })}
                </p>
              )}
            </div>
                ...
                {!selectedOfficialPreset && apiForm.type === "ollama" && (
                  <span className="text-slate-600">
                    ...
                    {t({
                      ...
                      ja: "ローカルでは通常不要",
                      zh: "本地环境通常不需要",
                      th: "usually not needed for local",
                    })}
                    )
                  </span>
                )}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {apiSaving
                  ? t({ ko: "저장 중...", en: "Saving...", ja: "保存中...", zh: "保存中..." , th: "กำลังบันทึก..." })
                  : apiEditingId
        ...
        {apiProvidersLoading ? (
          ...
          <div className="py-6 text-center text-xs text-slate-500">
            {t({
              ...
              th: "No API providers registered. Click + Add above to get started.",
            })}
          </div>
        ) : (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex flex-shrink-0 items-center gap-1.5">
                      <button
                        ...
                        title={t({ ko: "연결 테스트", en: "Test Connection", ja: "接続テスト", zh: "连接测试" , th: "ทดสอบการเชื่อมต่อ" })}
                      >
                  ...
                  {provider.models_cache && provider.models_cache.length > 0 && (
                    <div className="mt-2">
                      ...
                      >
                        {isExpanded ? "-" : "+"}{" "}
                        {t({ ko: "모델 목록", en: "Models", ja: "モデル一覧", zh: "模型列表" , th: "รายการโมเดล" })} (
                          ...
                          <div className="max-h-48 overflow-y-auto rounded border border-slate-700/30 bg-slate-900/40 p-2">
                            {filteredModels.map((model) => (
                              ...
                              >
                                ...
                                >
                                  {t({ ko: "배정", en: "Assign", ja: "割当", zh: "分配" , th: "มอบหมาย" })}
                                </button>
                              </div>
                            ))}
                            {filteredModels.length === 0 && (
                              <div className="text-[11px] text-slate-500 text-center py-2">
                                {t({ ko: "검색 결과 없음", en: "No results", ja: "結果なし", zh: "无结果" , th: "ไม่พบผลลัพธ์" })}
                              </div>
                            )}
                          </div>
                        </div>
